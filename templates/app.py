...
def generate_work_no(created_at):
    ...
    year = dt.strftime("%y")  # 26 (จาก 2026)

    # prefix สำหรับค้นหา เช่น %1026
    suffix = f"{month}{year}"

    # นับจำนวนงานของเดือน+ปีนี้
    cursor.execute(
        """
        SELECT COUNT(*)
        FROM reports
    ...
    # รวมเป็นเลขงาน 7 หลัก
    return f"{seq_str}{suffix}"
...
def list_reports():
    ...
    if date_from and date_to:
        # ผู้ใช้เลือกวันที่เอง → ใช้ช่วงวันที่
...
def assets_list():
    ...
    if dept:
        cursor.execute("""
            SELECT
                id, asset_no, asset_type, asset_model,
                serial_no, hostname, owner_name,
                department, status
            FROM assets
            WHERE department = ?
        ...
        # ⭐ ยังไม่เลือกหน่วยงาน → ไม่แสดงข้อมูล
        assets = []
        page_title = "กรุณาเลือกหน่วยงาน"
...
def export_assets_excel():
    ...
    if dept:
        ...
        cursor.execute("""
            SELECT
                ...
                department
            FROM assets
            ORDER BY department, asset_type, asset_no
        """)
    ...
    font_header = Font(bold=True, size=13)
    ...
    filename = f"assets_{dept_full}_{datetime.now().strftime('%Y%m%d')}.xlsx"
